...
@section('title', 'Создание рабочего процесса')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Создание рабочего процесса</h1>
                <p class="mt-2 text-gray-600">Настройте новый рабочий процесс для документооборота</p>
            </div>
        </div>
    </div>
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" 
                              ...
                              rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
                              placeholder="Краткое описание назначения рабочего процесса">{{ old('description') }}</textarea>
                </div>
                ...
                <div>
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm text-gray-700">Активировать процесс после создания</span>
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 bg-white shadow-sm rounded-lg overflow-hidden">
            ...
            <div class="p-6">
                <div id="states-container">
                    <div class="space-y-3">
                        @forelse(old('states', ['draft', 'in_progress', 'approved', 'completed']) as $index => $state)
                            <div class="state-item flex items-center space-x-3">
                                ...
                                <label class="flex items-center">
                                    <input type="radio" 
                                           ...
                                           value="{{ $index }}"
                                           {{ old('initial_state') === $state || ($index === 0 && !old('initial_state')) ? 'checked' : '' }}
                ...
                <input type="hidden" name="initial_state" id="initial_state" value="{{ old('initial_state', 'draft') }}">
            </div>
        </div>
        ...
        <div class="mt-6 bg-white shadow-sm rounded-lg overflow-hidden">
            ...
            <div class="p-6">
                <div id="transitions-container">
                    <div class="space-y-4">
                        @forelse(old('transitions', [
                            ['name' => 'submit', 'from' => ['draft'], 'to' => 'in_progress'],
                            ['name' => 'approve', 'from' => ['in_progress'], 'to' => 'approved'],
                            ...
                            <div class="transition-item border border-gray-200 rounded-lg p-4">
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Из состояний</label>
                                        <input type="text" 
                                               name="transitions[0][from_states]" 
                                               value="draft"
                                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
                                               required>
                                    </div>
...
<script>
let stateIndex = {{ count(old('states', ['draft', 'in_progress', 'approved', 'completed'])) }};
let transitionIndex = {{ count(old('transitions', [['name' => 'submit', 'from' => ['draft'], 'to' => 'in_progress']])) }};
...
document.getElementById('workflow-form').addEventListener('submit', function(e) {
    ...
    transitionItems.forEach((item, index) => {
        const fromStatesInput = item.querySelector(`input[name="transitions[${index}][from_states]"]`);
